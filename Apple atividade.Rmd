...
knitr::opts_knit$set(root.dir = "D:/Matheus/Apple")
```
# Mostrar o RStudio
...
dados_selecionados <- dados[, c("Último", "Abertura", "Máxima", "Mínima", "Vol.", "Var.")]
...
```{r grafico 2, echo = TRUE}
ggplot(dados, aes(x = Mínima, y = Máxima)) +
  geom_point(aes(color = "Pontos"), alpha = 0.5) +  
  geom_smooth(aes(color = "Linha de Regressão"), method = "lm", se = FALSE) +  
  scale_color_manual(name = "Legenda", values = c("Pontos" = "blue", "Linha de Regressão" = "red")) +  
  labs(title = "Relação entre Máxima e Mínima",
       x = "Mínima",
       y = "Máxima") +
  theme_classic()
```
...
ggplot(dados, aes(x = 1:nrow(dados))) +
  geom_line(aes(y = Máxima, color = "Máxima"), size = 1) + 
  geom_line(aes(y = Mínima, color = "Mínima"), size = 1) +
